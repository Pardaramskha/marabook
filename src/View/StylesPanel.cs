...
namespace Marabook.View
{
    ...
    public class StylesPanel : Grid
    {
        ...
        private DockPanel FormRow(string label, UIElement field)
        {
        ...
        private void Hook(UIElement element)
        {
            ...
            if (panel != null) foreach (UIElement child in panel.Children) Hook(child);
        }
        ...
        public static UIElement RowContent(ParagraphStyle style)
        {
        ...
        public static FrameworkElement ScopeIcon(ParagraphStyle style, double size)
        {
            string name = null;
            if (style.Scope == ParagraphStyle.ScopeBook) name = "book-bold";
            else if (style.Scope == ParagraphStyle.ScopeDocument) name = "document";
            if (name == null) return null;
            var icon = Icons.Make(name, size, Chrome.SoftText) as FrameworkElement;
            if (icon == null) return null;
            icon.VerticalAlignment = VerticalAlignment.Center;
            icon.Margin = new Thickness(0, 0, 5, 0);
            icon.ToolTip = style.Scope == ParagraphStyle.ScopeBook ? "Style du livre" : "Style du document";
            return icon;
        }
        ...
        private void DeleteStyle()
        {
            ...
            if (style.Id == "body")
            {
                MessageDialog.Show(Window.GetWindow(this), "Le style « Corps » est le style de secours : il ne peut pas être supprimé.",
